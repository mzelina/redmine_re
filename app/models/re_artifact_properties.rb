class ReArtifactProperties < ActiveRecord::Base
  ...
  has_many :related_diagrams,
           :through => :diagram_relations,
           :source => "sink"
           # :class_name => "ConcreteDiagram"
  ...
  has_many :traces_as_sink,
           -> { where("re_artifact_relationships.relation_type NOT IN (?)", ReArtifactRelationship::SYSTEM_RELATION_TYPES).order("re_artifact_relationships.position") },
           :foreign_key => "sink_id",
           :class_name => "ReArtifactRelationship",
           :dependent => :destroy
