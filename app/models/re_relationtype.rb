  ...
  def self.relation_types (project_id, is_system_relation=nil, is_used_relation=nil)
    ...
    if is_system_relation == nil
      if is_used_relation == nil
        ...
        tmp = ReRelationtype.where("project_id=? AND in_use=?", project_id, is_used_relation)
      end  
    else
      if is_used_relation == nil
        tmp = ReRelationtype.where("project_id=? AND is_system_relation=?", project_id, is_system_relation)
      else 
        tmp = ReRelationtype.where("project_id=? AND is_system_relation=? AND in_use=?", project_id, is_system_relation, is_used_relation)
      end
    end
